name 'aws_vpc_plugin_saopaulo'
type 'plugin'
rs_ca_ver 20161221
short_description "Amazon Web Services - VPC Plugin"
long_description "Version 1.4"
package "plugin/rs_aws_vpc_saopaulo"
import "sys_log"

resource_pool "rs_aws_vpc_saopaulo" do
  plugin $rs_aws_vpc_saopaulo
...
plugin "rs_aws_vpc_saopaulo" do
  endpoint do
    default_host "ec2.sa-east-1.amazonaws.com"
...
define provision_vpc(@declaration) return @vpc do
  sub on_error: stop_debugging() do
    ...
    @vpc = rs_aws_vpc_saopaulo.vpc.create($fields)
    call stop_debugging()
    $vpc = to_object(@vpc)
...
define provision_endpoint(@declaration) return @vpcendpoint do
  sub on_error: stop_debugging() do
    ...
    $name = $fields['name']
    call start_debugging()
    @vpcendpoint = rs_aws_vpc_saopaulo.endpoint.create($fields)
    call stop_debugging()
...
define provision_route_table(@declaration) return @resource do
  sub on_error: stop_debugging() do
    ...
    $name = $fields['name']
    call start_debugging()
    @resource = rs_aws_vpc_saopaulo.route_table.create($fields)
    call stop_debugging()
...
define provision_nat_gateway(@declaration) return @resource do
  sub on_error: stop_debugging() do
    ...
    $name = $fields['name']
    call start_debugging()
    @resource = rs_aws_vpc_saopaulo.nat_gateway.create($fields)
    call stop_debugging()
    $vpc = to_object(@resource)
...
define provision_tags(@declaration) return @resource do
  sub on_error: stop_debugging() do
    ...
    $name = $fields['name']
    call start_debugging()
    @resource = rs_aws_vpc_saopaulo.tags.create($fields)
    call stop_debugging()
    $vpc = to_object(@resource)
