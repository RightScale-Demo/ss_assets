name 'aws_vpc_plugin_sydney'
type 'plugin'
rs_ca_ver 20161221
short_description "Amazon Web Services - VPC Plugin"
long_description "Version 1.4"
package "plugin/rs_aws_vpc_sydney"
import "sys_log"

resource_pool "rs_aws_vpc_sydney" do
  plugin $rs_aws_vpc_sydney
...
plugin "rs_aws_vpc_sydney" do
  endpoint do
    default_host "ec2.ap-southeast-2.amazonaws.com"
...
define provision_vpc(@declaration) return @vpc do
  sub on_error: stop_debugging() do
    ...
    @vpc = rs_aws_vpc_sydney.vpc.create($fields)
    call stop_debugging()
    $vpc = to_object(@vpc)
...
define provision_endpoint(@declaration) return @vpcendpoint do
  sub on_error: stop_debugging() do
    ...
    $name = $fields['name']
    call start_debugging()
    @vpcendpoint = rs_aws_vpc_sydney.endpoint.create($fields)
    call stop_debugging()
...
define provision_route_table(@declaration) return @resource do
  sub on_error: stop_debugging() do
    ...
    $name = $fields['name']
    call start_debugging()
    @resource = rs_aws_vpc_sydney.route_table.create($fields)
    call stop_debugging()
...
define provision_nat_gateway(@declaration) return @resource do
  sub on_error: stop_debugging() do
    ...
    $name = $fields['name']
    call start_debugging()
    @resource = rs_aws_vpc_sydney.nat_gateway.create($fields)
    call stop_debugging()
    $vpc = to_object(@resource)
...
define provision_tags(@declaration) return @resource do
  sub on_error: stop_debugging() do
    ...
    $name = $fields['name']
    call start_debugging()
    @resource = rs_aws_vpc_sydney.tags.create($fields)
    call stop_debugging()
    $vpc = to_object(@resource)
