...
define build_rl_enablement_userdata($server_template_name, $server_name, $rs_refresh_token) return $user_data_base64 do
 ...
 $rl_enablement_cmd = 'curl -s https://rightlink.rightscale.com/rll/10/rightlink.enable.sh | bash -s -- -k "'+cred($rs_token_cred_name)+'" -t "'+$server_template_name+'" -n "'+$server_name+'" -d "'+@@deployment.name+'" -c "amazon"'
